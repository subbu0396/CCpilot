...
export function AISuggestions() {
  ...
  return (
    <section id="suggestions" className="scroll-mt-24">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Claude-ranked actions for the current filter state
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="rounded-xl border-2 border-[#2f6f6a]/30 bg-white p-4 shadow-md shadow-slate-200/60">
        {loading ? (
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {suggestions.map((s, i) => (
              <article
                key={`${s.headline}-${i}`}
                className={`rounded-lg border border-slate-200 border-l-4 bg-[#fbfaf7] p-4 ${borderColor[s.priority]}`}
              >
