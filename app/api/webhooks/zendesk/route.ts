...
/** Sets a Zendesk ticket to urgent priority + tags it when the Core Analysis Agent flags escalation. Best-effort: failures here never fail the webhook response. */
async function escalateZendeskTicket(ticketId: number | string) {
  ...
  try {
    ...
    await fetch(`https://${subdomain}.zendesk.com/api/v2/tickets/${ticketId}.json`, {
      ...
      body: JSON.stringify({
        ticket: { priority: "urgent", additional_tags: ["churn_risk_flagged"] },
      }),
    });
  } catch (err) {
