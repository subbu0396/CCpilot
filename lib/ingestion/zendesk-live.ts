 ...
 * Agent flags escalation. Best-effort: failures here never fail the caller's
 * larger operation (webhook response, sync result). Two separate calls on
 * purpose: `additional_tags` on the single-ticket PUT is silently ignored by
 * Zendesk (it's only honored on the bulk update_many endpoint) — tags must
 * go through the dedicated PUT /tickets/{id}/tags.json endpoint, which adds
 * without clobbering.
 */
export async function escalateZendeskTicket(ticketId: number | string): Promise<void> {
  ...
  try {
    ...
    console.error(`[zendesk] priority escalation failed for ticket ${ticketId}:`, err);
  }
}
